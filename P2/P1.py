...
def amigo_mas_cercano(dato, clase, datos, clases, indice):
    indices_amigos = np.where(clases == clase)[0]
    indices_amigos = np.delete(indices_amigos, np.where(indices_amigos == indice)[0])
...
def relief(datos, clases):
    ...
    for i, ei in enumerate(datos):
        ...
        ea = amigo_mas_cercano(ei, clases[i], datos, clases, i)
